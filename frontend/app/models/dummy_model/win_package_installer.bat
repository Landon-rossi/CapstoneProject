...
if not "%PY_VERSION_MAJOR_MINOR%"=="%REQUIRED_VERSION%" (
    echo WARNING: Python %REQUIRED_VERSION% not found (detected %PY_VERSION_MAJOR_MINOR%)
    echo Skipping Chocolatey install to avoid admin prompt or downgrade conflict.
    echo Attempting to continue with existing Python version...
) else (
...
if %ERRORLEVEL% NEQ 0 (
    echo FAILED: requirements install failed.
    exit /b 1
)

echo Install complete.
echo SUCCESS: Model environment is ready.
exit /b 0
